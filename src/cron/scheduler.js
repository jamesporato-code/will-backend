...
function startDailyCron() {
  ...
  logger.info('Cron planifie : messages quotidiens a 8h (lun-ven)');
}
...
async function sendDailyMessages() {
  try {
    ...
    for (const user of users) {
      ...
      } catch (err) { logger.error('Erreur envoi quotidien a ' + user.whatsapp_id, err.message); }
    }
    logger.info('Messages quotidiens envoyes');
  } catch (err) { logger.error('Erreur cron quotidien', err); }
}
