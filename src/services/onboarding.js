...
async function handleOnboarding(user, parsed) {
  ...
  if (step === 3 && parsed.buttonId?.startsWith('ob_goal_')) {
    ...
    await whatsapp.sendText(user.whatsapp_id, "Parfait ! Chaque jour je t'envoie un message personnalise sur l'IA.");
    await delay(1000);
    ...
    await whatsapp.sendList(
      ...
      [{
        ...
        rows: [
          ...
          { id: 'ob_hour_20', title: '20h00', description: 'En soiree' },
        ]
      }]
    );
    return true;
